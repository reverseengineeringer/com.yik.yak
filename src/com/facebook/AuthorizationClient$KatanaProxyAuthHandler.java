  ...
  extends AuthorizationClient.KatanaAuthHandler
{
  ...
  private AuthorizationClient.Result handleResultOk(Intent paramIntent)
  {
    ...
    if (!Utility.isNullOrEmpty(str1)) {
      AuthorizationClient.access$200(this$0, applicationId, str1);
    }
    if ((paramIntent == null) && (str2 == null) && (localObject == null))
    {
      paramIntent = AccessToken.createFromWebBundle(this$0.pendingRequest.getPermissions(), localBundle, AccessTokenSource.FACEBOOK_APPLICATION_WEB);
      return AuthorizationClient.Result.createTokenResult(this$0.pendingRequest, paramIntent);
    }
    if (ServerProtocol.errorsProxyAuthDisabled.contains(paramIntent)) {
      return null;
    }
    if (ServerProtocol.errorsUserCanceled.contains(paramIntent)) {
      return AuthorizationClient.Result.createCancelResult(this$0.pendingRequest, null);
    }
    return AuthorizationClient.Result.createErrorResult(this$0.pendingRequest, paramIntent, (String)localObject, str2);
  }
  ...
  boolean onActivityResult(int paramInt1, int paramInt2, Intent paramIntent)
  {
    if (paramIntent == null)
    {
      paramIntent = AuthorizationClient.Result.createCancelResult(this$0.pendingRequest, "Operation canceled");
      if (paramIntent == null) {
        break label86;
      }
      this$0.completeAndValidate(paramIntent);
    }
    for (;;)
    {
      return true;
      if (paramInt2 == 0)
      {
        paramIntent = AuthorizationClient.Result.createCancelResult(this$0.pendingRequest, paramIntent.getStringExtra("error"));
        break;
      }
      if (paramInt2 != -1)
      {
        paramIntent = AuthorizationClient.Result.createErrorResult(this$0.pendingRequest, "Unexpected resultCode from authorization.", null);
        break;
      }
  ...
  boolean tryAuthorize(AuthorizationClient.AuthorizationRequest paramAuthorizationRequest)
  {
    ...
    Intent localIntent = NativeProtocol.createProxyAuthIntent(this$0.context, paramAuthorizationRequest.getApplicationId(), paramAuthorizationRequest.getPermissions(), str, paramAuthorizationRequest.isRerequest(), paramAuthorizationRequest.getDefaultAudience());
