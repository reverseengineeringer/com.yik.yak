  ...
  implements IRequestHandler
{
  ...
  private void closePackage(ActivityPackage paramActivityPackage, String paramString, Throwable paramThrowable)
  {
    ...
    logger.error("%s. (%s) %s", new Object[] { str1, paramString, str2 });
    paramString = ResponseData.fromError(paramString);
    if (!packageHandler.dropsOfflineActivities()) {}
    for (;;)
    {
      paramString.setWillRetry(bool);
      packageHandler.finishedTrackingActivity(paramActivityPackage, paramString, null);
  ...
  private String parseResponse(HttpResponse paramHttpResponse)
  {
    ...
    {
      logger.error("Failed to parse response (%s)", new Object[] { paramHttpResponse });
    }
  ...
  private void requestFinished(HttpResponse paramHttpResponse, ActivityPackage paramActivityPackage)
  {
    ...
    Object localObject = parseResponse(paramHttpResponse);
    paramHttpResponse = Util.buildJsonObject((String)localObject);
    localObject = ResponseData.fromJson(paramHttpResponse, (String)localObject);
    if (200 == i)
    {
      ((ResponseData)localObject).setWasSuccess(true);
      logger.info(paramActivityPackage.getSuccessMessage(), new Object[0]);
    }
    for (;;)
    {
      packageHandler.finishedTrackingActivity(paramActivityPackage, (ResponseData)localObject, paramHttpResponse);
      packageHandler.sendNextPackage();
      return;
      logger.error("%s. (%s)", new Object[] { paramActivityPackage.getFailureMessage(), ((ResponseData)localObject).getError() });
    }
  }
  ...
  private void sendNextPackage(ActivityPackage paramActivityPackage, String paramString, Throwable paramThrowable)
  {
    ...
    paramString = getReasonString(paramString, paramThrowable);
    logger.error("%s. (%s)", new Object[] { str, paramString });
    paramString = ResponseData.fromError(paramString);
    packageHandler.finishedTrackingActivity(paramActivityPackage, paramString, null);
    packageHandler.sendNextPackage();
  }
