  ...
  public BackStackRecord instantiate(FragmentManagerImpl paramFragmentManagerImpl)
  {
    ...
    while (i < mOps.length)
    {
      ...
      if (FragmentManagerImpl.DEBUG) {
        new StringBuilder().append("Instantiate ").append(localBackStackRecord).append(" op #").append(k).append(" base fragment #").append(mOps[j]).toString();
      }
      localObject = mOps;
      i = j + 1;
      j = localObject[j];
      ...
      for (fragment = ((Fragment)mActive.get(j));; fragment = null)
      {
        ...
        for (;;)
        {
          ...
          if (FragmentManagerImpl.DEBUG) {
            new StringBuilder().append("Instantiate ").append(localBackStackRecord).append(" set remove fragment #").append(mOps[j]).toString();
          }
